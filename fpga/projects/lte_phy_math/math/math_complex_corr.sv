...
module math_complex_corr #(
    ...
    wire op_valid_data = `AND(i_data1_valid, i_data2_valid);
    ...
    assign op_valid_it   = `AND(
        `AND(op_val_mul_ii, op_val_mul_qq),
        `AND(op_val_mul_iq, op_val_mul_qi)
    );

    assign op_last_it    = `AND(op_valid_it, `EQ(op_corr_w_idx, LAST_IDX));
