...
module math_fma_macro #(
    ...
    // конвейер обработки ДО ОПЕРАЦ�?Й A * B и A * B + C
    ...
    reg signed [AB_mul_size - 1:0]   mul_ff;
    ...
    always @(posedge clk) begin
        if (rst) begin 
            ...
            mul_ff <= $signed(pipe_a[PIPE - 1]) * $signed(pipe_b[PIPE - 1]);
            res_ff <= $signed(mul_ff) + $signed(pipe_c[PIPE]);
        end 
    end
    ...
    assign ab_mul  = ~pipe_i_valid[PIPE] ? 0 : mul_ff;
