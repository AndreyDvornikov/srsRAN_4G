...
module lte_phy_pss_detector #(
    ...
    output reg  [31:0]                        o_shift,       // absolute END sample index
    output wire                               o_busy,
    ...
    // old implementation:
    //   rx[start + tap_idx] * coef[tap_idx]
    //
    // Here:
    //   oldest sample of current window = win_[PSS_LEN-1]
    //   so tap 0 reads oldest sample, tap 1 next oldest, etc.
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            ...
            if (sample_accept) begin
                ...
                // absolute index of accepted sample
            ...
            case (sst)
                ...
                S_OUT: begin
                    ...
                    o_pss_idx   <= best_idx;
                    o_shift     <= job_end_abs;   // END index of current window
                    o_pss_valid <= 1'b1;
