...
//  - внутри: ROM load PSS0/PSS1/PSS2, ping-pong capture 5ms, block-scan
//  - K_LANES = число параллельных стартов окна (k..k+K_LANES-1)
//  - использует math_complex_corr (sum(rx * conj(pss)))
//  - o_shift = PEAK(end) absolute (0-based)
//  - o_dbg_mag_pss0/1/2 = лучшие магнитуды для PSS0/PSS1/PSS2
// ============================================================================
module lte_pss_detector #(
    ...
    localparam int BUF_LEN = (`HW_ADC_TARGET_FS / 200);     // 5ms samples
    ...
    // =========================================================================
    // abs sample counter (counts ALL i_valid samples always)
    // =========================================================================
    reg [31:0] abs_sample;
    ...
    // =========================================================================
    // Ping-pong capture buffers
    // =========================================================================
    (* ram_style="block" *) reg signed [ADC_W-1:0] frame0_i [0:BUF_LEN-1];
    (* ram_style="block" *) reg signed [ADC_W-1:0] frame0_q [0:BUF_LEN-1];
    (* ram_style="block" *) reg signed [ADC_W-1:0] frame1_i [0:BUF_LEN-1];
    (* ram_style="block" *) reg signed [ADC_W-1:0] frame1_q [0:BUF_LEN-1];

    reg               wr_buf;
    reg [BUF_AW-1:0]  wr_idx;
    reg [31:0]        buf_abs0 [0:1];
    reg               buf_ready[0:1];
    reg [31:0]        dropped_frames;

    always @(posedge i_clk) begin
        if (i_rst) begin
            wr_buf <= 1'b0;
            wr_idx <= '0;
            buf_abs0[0] <= 32'd0; buf_abs0[1] <= 32'd0;
            buf_ready[0] <= 1'b0; buf_ready[1] <= 1'b0;
            dropped_frames <= 32'd0;
        end else if (i_valid) begin
            if (wr_idx == 0) buf_abs0[wr_buf] <= abs_sample;

            if (!wr_buf) begin
                frame0_i[wr_idx] <= i_data_i1;
                frame0_q[wr_idx] <= i_data_q1;
            end else begin
                frame1_i[wr_idx] <= i_data_i1;
                frame1_q[wr_idx] <= i_data_q1;
            end

            if (wr_idx == (BUF_LEN-1)) begin
                buf_ready[wr_buf] <= 1'b1;

                if (buf_ready[~wr_buf]) begin
                    dropped_frames <= dropped_frames + 1'b1;
                    buf_ready[~wr_buf] <= 1'b0;
                end

                wr_buf <= ~wr_buf;
                wr_idx <= '0;

                $display("[%0t] CAP done: BUF_LEN=%0d abs0=%0d (wr->%0d) dropped=%0d",
                         $time, BUF_LEN, buf_abs0[wr_buf], ~wr_buf, dropped_frames);
            end else begin
                wr_idx <= wr_idx + 1'b1;
            end
        end
    ...
    wire have_ready = buf_ready[0] || buf_ready[1];
    wire pick_buf0  = buf_ready[0] && (!buf_ready[1] || (buf_abs0[0] <= buf_abs0[1]));
    wire pick_buf1  = buf_ready[1] && (!buf_ready[0] || (buf_abs0[1] <  buf_abs0[0]));
    ...
    generate
        for (gv=0; gv<K_LANES; gv=gv+1) begin : gen_lane_en
            ...
            assign feed_lane[gv] = (sst == S_FEED) && lane_en[gv];
        end
    endgenerate
    ...
    generate
        for (gv=0; gv<K_LANES; gv=gv+1) begin : gen_lane_rx
            ...
            wire signed [ADC_W-1:0] rx_i = (!scan_buf) ? frame0_i[idx] : frame1_i[idx];
            wire signed [ADC_W-1:0] rx_q = (!scan_buf) ? frame0_q[idx] : frame1_q[idx];
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            ...
            case (sst)
                S_IDLE: begin
                    if (pss_loaded && have_ready) begin
                        if (pick_buf0) begin
                            scan_buf <= 1'b0;
                            scan_abs0 <= buf_abs0[0];
                            buf_ready[0] <= 1'b0;
                        end else if (pick_buf1) begin
                            scan_buf <= 1'b1;
                            scan_abs0 <= buf_abs0[1];
                            buf_ready[1] <= 1'b0;
                        end
                        sst <= S_INIT;
                    end
                ...
                S_OUT: begin
                    ...
                    $display("[%0t] PEAK(K_LANES=%0d): PSS0 start=%0d peak=%0d mag=%0d | PSS1 start=%0d peak=%0d mag=%0d | PSS2 start=%0d peak=%0d mag=%0d | WIN=%0d peak_abs=%0d | period=%0d | dropped=%0d",
                             $time, K_LANES,
                             ...
                             win_idx, win_peak_abs, pss_period_sps, dropped_frames);
