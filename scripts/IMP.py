...
MOVING_AVG_WINDOW = 5
CDF_MAX_SAMPLES = 10000  # ограничение накопленных точек для CDF
...
class DashboardApp:
    def __init__(self, root: tk.Tk, metrics_path: str):
        ...
        self.root.geometry("1320x900")  # окно достаточно высокое
        self.root.configure(bg=CLR_BG)

        # Истории для графиков (существующие)
        ...
        # Новые хранилища для дополнительных графиков
        self.total_dl_throughput_history = deque(maxlen=MAX_POINTS)  # Мбит/с
        self.se_cdf_samples: list[float] = []  # все значения SE для CDF
        self.ue_cqi_histories: dict[int, deque] = {}  # ключ RNTI -> deque CQI
    ...
    def _build_layout(self):
        ...
        # Графики — теперь Notebook с двумя вкладками
        ...
        # Вкладка 1: Планировщик (существующие три графика)
        ...
        # Вкладка 2: Общая / CQI (новые графики)
        tab2 = ttk.Frame(self.notebook)
        self.notebook.add(tab2, text="Общая / CQI")
        self.figure2 = Figure(figsize=(8, 6.5), dpi=100, facecolor=CLR_SURFACE)
        self.tput_ax = self.figure2.add_subplot(311)      # Суммарный DL Throughput
        self.cdf_se_ax = self.figure2.add_subplot(312)    # CDF спектральной эффективности
        self.cqi_ax = self.figure2.add_subplot(313)       # Динамика DL CQI
    ...
    def _poll_metrics(self):
        ...
        active_throughputs, total_throughput, total_prb, avg_mcs, ue_data_list = (
            self._update_aggregated_metrics(mac.get("ue_list", []), latency_map)
        )
        self._update_general_metrics(mac)

        # PRB Util (из JSON или вычисляем)
        ...
        if prb_util_raw is not None:
            ...
            if nof_prb and nof_prb > 0:
                prb_sum = sum(
                    c.get("mac_ue_container", c.get("ue_container", {})).get("dl_prb", 0)
        ...
        # === Новые сборы для дополнительных графиков ===

        # Суммарный DL Throughput (Мбит/с)
        total_tput_bps = sum(
            ue["dl_throughput"] for ue in ue_data_list if ue["dl_throughput"] is not None
        )
        self.total_dl_throughput_history.append(total_tput_bps / 1e6)  # переводим в Мбит/с

        # CDF SE: собираем индивидуальные SE
        ...
        # Удаляем истории для RNTI, которые давно не появлялись (не обязательно)
    ...
    def _update_aggregated_metrics(self, ue_list, latency_map):
        ...
        for entry in ue_list:
            ...
            raw = {
                ...
                "dl_cqi": to_number(c.get("dl_cqi")),  # нужно для CQI графика
            }
            ...
            ue_info = {
                ...
                "dl_cqi": raw["dl_cqi"],  # сохраняем CQI
        ...
        # Агрегация (заполнение self.aggregated_metrics)
        count = len(ue_data)
    ...
    def _redraw_plots(self):
        ...
        # CDF спектральной эффективности
