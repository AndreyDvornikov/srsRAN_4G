...
1. libtool ```sudo apt install libtool```
2. pkg-config ```sudo apt install pkg-config```
3. build-essentials ```sudo apt-get install build-essential```
3. (for doc)  ```sudo apt-get install asciidoc xmlto```
...
1. Download libzmq 4.3.5 source code
2. Configure project via bash

> WARNING! EDIT --prefix path

```
./configure --prefix=/home/d-moskovskikh/build/zmq-4.3.5/linux/x86_64/
```
